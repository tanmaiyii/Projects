...
load('data/global/all_features');
DictionarySize = 500;
%run('software/vlfeat-master/toolbox/vl_setup');% to compile the vlfeat lab. 
tic
...
wordid =78; %397; %37;% set a random value and we intead to find 20 patches from both training and test(size should be less than C)
...
%     % features for each image loaded 900x128
    % calculate the distance 900x500
    % get the index of the closest word for each patch 900x1
    ...
    for cj = 1:num_class
        confusion_matrix(ci,cj)=...
            size(find(predict_label((ci-1)...
                      *num_test_1c+1:ci*num_test_1c,:)==cj),1)...
            /num_test_1c;
    end 
end 
close all; figure;
...
right_v = find(predict_label == test_labels);%correct
wrong_v = find(predict_label ~= test_labels);%incorrect 
idright = right_v(6);% set number you are about to visualize
idwrong = wrong_v(10);% set number you are about to visualize, 
%be sure it is nor larger than length of wrong_v 
gt_label_r = class_names{test_labels(idright)};
gt_label_w = class_names{test_labels(idwrong)};
prd_label_r = class_names{predict_label(idright)};
prd_label_w = class_names{predict_label(idwrong)};
...
%% represent each image using BoW when now C is 20
%
clear;
BoW =[]; %initialization 
isshow = 1; % show image and histogram or not
...
clear BoW;
k = 1; % set the k for k-nn algorithm 
method = 2; % 1-L2; 2- Histogram intersection %change the method to 2 after testing for 1.
NNresult = knnsearch(test_data, train_data, k, method);
...
wrong_v = find(predict_label ~= test_labels);%incorrect 
idright = right_v(2);% set number you are about to visualize
idwrong = wrong_v(1);% set number you are about to visualize, 
%be sure it is nor larger than length of wrong_v 
gt_label_r = class_names{test_labels(idright)};
gt_label_w = class_names{test_labels(idwrong)};
prd_label_r = class_names{predict_label(idright)};
...
% visualize using step 2.5 The steps from 2.4 and 2.5 are repeated with
% C=20
% step 2.4 and 2.5 are repeated for this process
%==========================================================================
%% 2.4 Assign each descriptor to the nearest codeword 
...
clear;
load('data/global/assignd_discriptor');
wordid =8; %15; %10;% set a random value and we intead to find 20 patches from both training and test, (size should be less than C)
n = 30;% number of patches
close all;
visualize_patches(index_train,index_test,wordid,n);
clear; 
